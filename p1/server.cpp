...
int main(int argc, char *argv[])
{
  ...
  cout << "before blocking call! " << endl;
  
  while ((bytes_read = recvfrom(udp_socket, &p, sizeof(p) + ntohs(p.length), 0, (struct sockaddr *) &client_sockaddr,&client_length)) >= 0)
    {
      cout << "recieved: " << (ntohl(p.sequence_number));
  ...
  if(!(bytes_read > (size_t)MAXIMUM_DATA_LENGTH)){
    ...
    send_back_packet.sequence_number = htonl(p.sequence_number);
    ...
    cout << "packet in server has been sent!" << endl;
    ...
    {
      cout<< "The packet #" << ntohl(p.sequence_number) << " was dropped because it exceeded maximum packet size" << endl;
    }
      }
      else{
  cout << "The packet #" << ntohl(p.sequence_number) << " was dropped because the command 'D' was not issued" << endl;
      }
      //Removed a }
      memset(&p, 0, sizeof(p));
    }
